...
def progress(prefix, percent):
    bar_length = 30
    hashes = '#' * int(percent / 100.0 * bar_length)
    spaces = ' ' * (bar_length - len(hashes))
    sys.stdout.write("\r[%s]: |%s%s| %d%%" % (prefix, hashes, spaces, percent))
    sys.stdout.flush()
...
class TftpContextClientUpload(TftpContext):
    ...
    def start(self):
        ...
        cnt = 0
        total = os.path.getsize(self.file_path) / 51200.0
        progress(self.host, 0)
        while self.state:
            try:
                log.debug("State is %s" % self.state)
                self.cycle()
                progress(self.host, cnt/total)
                cnt += 1
